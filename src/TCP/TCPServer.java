...
public class TCPServer {
		...
		try {
			...
			while (true) { // Server laufen IMMER
				System.out.println("TCP Server is waiting for connection - listening TCP port " +
						serverPort);
				/*
				 * Blockiert auf Verbindungsanfrage warten --> nach Verbindungsaufbau Standard-Socket
				 * erzeugen und an connectionSocket zuweisen
				 */
	...
	public static void main(String[] args) {
		...
		TCPServer myServer = new TCPServer(1331);
...
class TCPWorkerThread extends Thread {
	...
	public void run() {
		...
		System.out.println("TCP Worker Thread " + name + " is running until QUIT is received!");
		...
		try {
			...
			System.err.println("Connection aborted by client!" + e.getMessage());
		} finally {
	...
	private void writeToClient(String reply) throws IOException {
		...
		System.out.println("TCP Worker Thread " + name + " has written the message: " + reply);
	}
}
